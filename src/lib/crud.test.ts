...
interface MockDeps {
  ...
  getMutationId: ReturnType<typeof vi.fn>;
  logAction: ReturnType<typeof vi.fn>;
}

function createMockDeps(mockModel: MockModel): MockDeps {
  return {
    db: { testEntity: mockModel },
    broadcast: vi.fn(),
...
function mockReqRes(overrides: Partial<Request> = {}): { req: Request; res: MockRes } {
  const req = {
    query: {},
  ...
  res.json.mockImplementation((data: any) => { res._data = data; return res; });
  return { req, res };
}

function buildCrud(mockModel: MockModel, deps: MockDeps, overrides: Record<string, any> = {}) {
  return makeCrud({
    model: 'testEntity',
    entityName: 'test',
    createSchema,
...
describe('makeCrud', () => {
  let crud: ReturnType<typeof buildCrud>;
  let model: MockModel;
  let deps: MockDeps;
  ...
  describe('list', () => {
    it('returns paginated results with default limit', async () => {
      ...
      await crud.list(req, res as any, vi.fn());
    ...
    it('respects cursor parameter', async () => {
      const { req, res } = mockReqRes({ query: { cursor: 'abc-123' } } as any);
      await crud.list(req, res as any, vi.fn());
    ...
    it('caps limit at max', async () => {
      const { req, res } = mockReqRes({ query: { limit: '500' } } as any);
      await crud.list(req, res as any, vi.fn());
  ...
  describe('getById', () => {
    it('returns record when found', async () => {
      ...
      const { req, res } = mockReqRes();
      await crud.getById(req, res as any, vi.fn());
  ...
  describe('create', () => {
    it('creates record and broadcasts', async () => {
      ...
      const { req, res } = mockReqRes({ body: { name: 'New Thing' } } as any);
      await crud.create(req, res as any, vi.fn());
    ...
    it('returns 400 on validation error', async () => {
      const { req, res } = mockReqRes({ body: { name: '' } } as any);
      await crud.create(req, res as any, vi.fn());
  ...
  describe('update', () => {
    it('updates record and broadcasts', async () => {
      ...
      const { req, res } = mockReqRes({ body: { name: 'Updated' } } as any);
      await crud.update(req, res as any, vi.fn());
  ...
  describe('remove (soft delete)', () => {
    it('soft-deletes by setting deletedAt', async () => {
      const { req, res } = mockReqRes();
      await crud.remove(req, res as any, vi.fn());
  ...
  describe('approve', () => {
    it('sets approved=true and broadcasts', async () => {
      ...
      const { req, res } = mockReqRes();
      await crud.approve(req, res as any, vi.fn());
...
describe('makeCrud with mapListItem', () => {
  it('transforms list items', async () => {
    ...
    const crud = buildCrud(model, deps, {
      mapListItem: (item: any) => ({ id: item.id, name: item.name }),
    });
...
describe('makeCrud with beforeCreate hook', () => {
  it('transforms data before insert', async () => {
    ...
    const crud = buildCrud(model, deps, {
      beforeCreate: async (data: any) => ({ ...data, name: data.name.toUpperCase() }),
    });

    const { req, res } = mockReqRes({ body: { name: 'lower' } } as any);
    await crud.create(req, res as any, vi.fn());
