...
async function renderComments(targetType: string, targetId: string, container: HTMLElement): Promise<void> {
  let comments: any[] = [];
  try {
  ...
  container.innerHTML = html`
    <div class="comments-section">
      ...
      <div class="comment-list">
        ...
        ${raw(comments.map((c: any) => renderComment(c)).join(''))}
      </div>
...
onAction('postComment', (el: HTMLElement) => postComment(el.dataset.targetType!, el.dataset.id!));
onAction('deleteComment', (el: HTMLElement) => deleteComment(el.dataset.id!));

// ---- Reactive subscriptions ----

S.subscribe('_commentUpdate', (update: any) => {
  if (!update) return;
