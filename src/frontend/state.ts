...
var NAV_SCREENS: Array<{ id: string; label: string; icon: string; adminOnly?: boolean }> = [
...
var TASK_STATUSES: Record<string, { label: string; color: string }> = {
...
var STATUS_CYCLE: Record<string, string> = { todo: 'in_progress', in_progress: 'done', done: 'todo' };

var PROJECT_STATUSES = ['active', 'completed', 'archived'];

var PROJECT_TIERS: Record<string, { label: string; color: string; bg: string; description: string }> = {
...
var JOIN_TYPES: Record<string, { label: string; color: string; bg: string }> = {
...
var _subscribers: Record<string, Array<(value: any, old: any, key: string) => void>> = {};

var _stateData: Record<string, any> = {
  screen: 'dashboard',
  isAdmin: false,
  adminEmail: null,
...
var S: any = new Proxy(_stateData, {
...
S.subscribe = function(key: string, callback: (value: any, old: any, key: string) => void): () => void {
...
S.batch = function(updates: Record<string, any>): void {
  const changed: Array<{ key: string; value: any; old: any }> = [];
