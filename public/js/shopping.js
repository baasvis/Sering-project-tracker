...
function renderShoppingSection(items, projectId, options = {}) {
    ...
    var products = items.filter((i) => i.type === 'product' && i.approved);
    var costs = items.filter((i) => i.type === 'cost' && i.approved);
    ...
    if (products.length > 0) {
      ...
      <div class="shopping-table-head ${S.isAdmin ? 'has-actions' : ''}">
        ...
        for (var item of products) {
            ...
            var meta = [];
            if (item.importance)
                meta.push(html `<span class="badge badge-${raw(item.importance === 'Critical' ? 'danger' : item.importance === 'Medium' ? 'warning' : 'muted')}">${item.importance}</span>`);
            if (item.assignedTo)
...
async function saveShoppingItem(type, projectId, id) {
    ...
    if (name.length > 200)
        return toast('Name must be under 200 characters', 'error');
    var category = document.getElementById('shop-category').value.trim();
    var importance = document.getElementById('shop-importance').value;
    var assignedTo = document.getElementById('shop-assigned').value.trim();
    ...
    if (type === 'product') {
        var price = document.getElementById('shop-price').value;
        var qty = document.getElementById('shop-qty').value;
        data.pricePerItem = price ? parseFloat(price) : null;
        data.quantity = qty ? parseInt(qty) : 1;
        var link = document.getElementById('shop-link').value.trim();
        data.link = link || null;
    }
    else {
        var amount = document.getElementById('shop-amount').value;
