...
function safeDataImageSrc(str) {
    if (typeof str !== 'string')
        return '';
    if (/^data:image\/(png|jpeg|jpg|gif|webp|svg\+xml);base64,[A-Za-z0-9+/=]+$/.test(str))
        return str;
    return '';
}
...
async function apiFetch(method, url, body) {
    const opts = { method, cache: 'no-cache' };
    const headers = {};
    if (body !== undefined) {
    ...
    if (['POST', 'PATCH', 'PUT', 'DELETE'].includes(method)) {
        ...
        setTimeout(() => S._pendingMutationIds.delete(mutationId), 30000);
    }
...
function truncateDescription(descHtml, maxSentences = 3) {
    if (!descHtml)
        return '';
...
var TIER_ORDER = { mvp: 0, medium: 1, next_level: 2 };
